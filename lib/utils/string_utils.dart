...
class StringUtils {
  ...
  static String formatYr(LocaleService localeService, int yr) {
    if (yr == 0) yr = 1;
    return localeService.strings
        .yearFormat(yr.abs().toString(), getYrSuffix(localeService, yr));
  }

  static String getYrSuffix(LocaleService localeService, int yr) =>
      yr < 0 ? localeService.strings.yearBCE : localeService.strings.yearCE;

  static String getEra(LocaleService localeService, int yr) {
    if (yr <= -600) {
      return localeService.strings.eraPrehistory;
    }
    if (yr <= 476) return localeService.strings.eraClassical;
    if (yr <= 1450) {
      return localeService.strings.eraEarlyModern;
    }
    return localeService.strings.eraModern;
  }
