...
class AppStyle {
  ...
  /// Text styles
  late final _Text text = _Text(ServiceRegistry.I.localeService, scale);
...
@immutable
class _Text {
  _Text(this._localeService, this._scale);
  ...
  TextStyle _getFontForLocale(Map<String, TextStyle> fonts) {
    if (_localeService.isLoaded) {
      return fonts.entries
          .firstWhere((x) => x.key == _localeService.strings.localeName,
